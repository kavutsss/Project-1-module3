function ProjectList({ projects, onDeleteProject }) {
  ...
  if (safeProjects.length === 0) {
    return (
      <div className="bg-white p-16 rounded-xl text-center shadow-sm">
        <p className="text-gray-500 text-xl">No projects yet. Add your first one to get started!</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <h2 className="text-gray-700 text-2xl font-bold mb-4">Your Projects ({safeProjects.length})</h2>
          ...
          return (
            <div key={project.id} className="bg-white p-6 rounded-xl shadow-sm transition-colors hover:shadow-md flex flex-col">
              <div className="flex justify-between items-start mb-3 gap-2">
                <h3 className="text-gray-800 text-xl font-semibold flex-1 break-words">{project.title || 'Untitled Project'}</h3>
                {project.category && (
              ...
              <div className="flex justify-between items-center pt-4 border-t border-gray-200">
                ...
                <button 
                  onClick={() => project.id && typeof onDeleteProject === 'function' && onDeleteProject(project.id)}
